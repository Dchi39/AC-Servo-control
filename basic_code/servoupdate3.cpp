...
#define PWM_PERIOD 720  // As set in your CubeMX Counter Period
...
volatile uint8_t rotation_flag = 0;
float filteredADC = 0;
float targetFreq = 1.0f;
volatile float currentFreq = 1.0f;
...
volatile float speed = 0.05f;

void UpdateMotor(void) {
	    // 1. Calculate base indices
		int u_idx = ((int)angle) % SINE_STEPS;
	    int v_idx = u_idx + 120;
	    int w_idx = u_idx + 240;
...
int main(void)
{
  ...
  // --- START ALL 3 PHASES ---
          ...
          __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, PWM_PERIOD * 0.05f);
          __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, PWM_PERIOD * 0.05f);
          __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, PWM_PERIOD * 0.05f);
  ...
  while (1)
  {
	  ...
	  filteredADC = filteredADC * 0.9f + adcValue * 0.1f;
	  targetFreq = 1.0f + (filteredADC / 4095.0f) * 15.0f;
	  ...
	  speed = (currentFreq * SINE_STEPS) / 10000.0f;
	  ...
	  if (now - last_debug_print >= 200)
	  	  	 {
	  	  	  	...
	  	  	  	uint32_t encoder_pos = __HAL_TIM_GET_COUNTER(&htim2);
	  	  	  	uint8_t direction = __HAL_TIM_IS_TIM_COUNTING_DOWN(&htim2);

	  	  	int freq_int = (int)(currentFreq * 100);

	  	  	snprintf(buffer, sizeof(buffer),
	  	  	         "ADC: %u | Freq: %d.%02d Hz | Pos: %lu | Dir: %d | Revs: %lu\r\n",
	  	  	         adcValue,
	  	  	         freq_int / 100,
	  	  	         freq_int % 100,
	  	  	         encoder_pos,
	  ...
	  if (rotation_flag)
	  	  	{
	  	  	 snprintf(buffer, sizeof(buffer), "--- FULL ROTATION #%lu ---\r\n", revolution_count);
	  	  	 debug_print(buffer);
	  	  	 rotation_flag = 0; // Clear the flag
	  	  	}
    /* USER CODE END WHILE */
...
{
    if (GPIO_Pin == GPIO_PIN_2) // Check if the interrupt came from PA2
    {
        revolution_count++;
        rotation_flag = 1; // Set a flag to notify the main loop

        /* Optional: Reset the Timer counter to 0 to align
           the encoder position with the physical index mark */
