...
/* This callback is called every time the Z-pulse triggers */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  static uint32_t last_motor_update = 0;
	  	    ...
	  	    char buffer[80]; // Declared here so both blocks can see it

	  	    // 1. HIGH PRIORITY: Update motor every 1ms
	  	    if (now - last_motor_update >= 1)
	  	    {
	  	        last_motor_update = now;
	  	        UpdateMotor();
	  	    }

	  	    // 2. LOW PRIORITY: Print debug info every 200ms
	  	    if (now - last_debug_print >= 200)
	  	    {
	  	        ...
	  	        snprintf(buffer, sizeof(buffer), "Pos: %lu | Dir: %d | Revs: %lu\r\n",
	  	                 encoder_pos, direction, revolution_count);
	  	        debug_print(buffer);
	  	    }

	  	    // 3. EVENT PRIORITY: Immediate print on Z-pulse detection
	  	    if (rotation_flag)
	  	    {
